...
var fluidPlayerClass = {
		...
		if (typeof creative !== 'undefined' && creative.length) {
			...
			if (
				...
				if (!player.hasVastAdTagUri(xmlResponse) && player.hasInLine(xmlResponse)) {
					...
					tmpOptions.skipoffset = player.convertTimeStringToSeconds(
	...
	processUrl: function(vastTag, adListId) {
		...
		player.resolveVastTag(vastTag, numberOfRedirects, adListId, tmpOptions);
	},
	...
	resolveVastTag: function(vastTag, numberOfRedirects, adListId, tmpOptions) {
		...
		var handleXmlHttpReq = function() {
			...
			if (
				numberOfRedirects > player.displayOptions.vastOptions.maxAllowedVastTagRedirects &&
				!player.inLineFound
			) {
				player.stopProcessAndReportError(adListId);
				return;
	...
	playRoll: function(adListId) {
		...
		var playVideoPlayer = function(adListId) {
			player.switchPlayerToVastMode = function() {
				...
				var addClickthroughLayer =
					typeof player.adList[adListId].adClickable != 'undefined'
						? player.adList[adListId].adClickable
						: player.displayOptions.vastOptions.adClickable;
				...
				if (player.displayOptions.layoutControls.layout !== 'browser') {
					var progressbarContainer = document.getElementById(
						player.videoPlayerId + '_fluid_controls_progress_container'
					);

					if (progressbarContainer !== null) {
						document.getElementById(
							player.videoPlayerId + '_vast_control_currentprogress'
						).style.backgroundColor =
							player.displayOptions.layoutControls.adProgressColor;
					}
				}

				if (player.displayOptions.vastOptions.adText || player.adList[adListId].adText) {
					var adTextToShow =
						player.adList[adListId].adText !== null
							? player.adList[adListId].adText
							: player.displayOptions.vastOptions.adText;
	...
	createNonLinearStatic: function(adListId) {
		...
		var duration = player.adList[adListId].nonLinearDuration
			? player.adList[adListId].nonLinearDuration
			: player.vastOptions.duration;
	...
	midRoll: function(event) {
		...
		if (
			player.displayOptions.vastOptions.showProgressbarMarkers &&
	...
	switchToMainVideo: function() {
		...
		var newCurrentTime =
			typeof videoPlayerTag.mainVideoCurrentTime !== 'undefined' ? videoPlayerTag.mainVideoCurrentTime : 0;
		...
		if (player.displayOptions.layoutControls.layout !== 'browser') {
			...
			if (progressbarContainer !== null) {
				backgroundColor = player.displayOptions.layoutControls.primaryColor
					? player.displayOptions.layoutControls.primaryColor
					: 'white';
				document.getElementById(
					player.videoPlayerId + '_vast_control_currentprogress'
	...
	onMainVideoEnded: function() {
		...
		if (player.displayOptions.layoutControls.loop === true) {
            var eventEnded = new CustomEvent('ended');
            videoPlayerTag.dispatchEvent(eventEnded);
	...
	addSkipButton: function() {
		...
		divSkipButton.innerHTML = this.displayOptions.vastOptions.skipButtonCaption.replace(
			'[seconds]',
			this.vastOptions.skipoffset
		);
	...
	addAdCountdown: function() {
		...
		// Create element
		var adCountdown =
			this.pad(parseInt(this.currentVideoDuration / 60)) +
			':' +
			this.pad(parseInt(this.currentVideoDuration % 60));
		...
		if (adPlayingDiv !== null) {
			var adPlayingDivPosition =
				adListData.adTextPosition !== null
					...
					: this.displayOptions.vastOptions.adTextPosition.toLowerCase();
	...
	addCTAButton: function(landingPage) {
		...
		link.innerHTML =
			this.displayOptions.vastOptions.adCTAText +
	...
	decreaseSkipOffset: function decreaseSkipOffset() {
		...
		if (btn) {
			if (sec >= 1) {
				...
				btn.innerHTML =
					'<a href="javascript:;" id="skipHref_' +
					player.videoPlayerId +
					'" onclick="fluidPlayerClass.getInstanceById(\'' +
					player.videoPlayerId +
					'\').pressSkipButton();">' +
	...
	addClickthroughLayer: function() {
		...
		divClickThrough.setAttribute(
			...
			'position: absolute; cursor: pointer; top: 0; left: 0; width: ' +
				videoPlayerTag.offsetWidth +
				'px; height: ' +
				videoPlayerTag.offsetHeight +
				'px;'
		);
		...
		var isIos9orLower =
			player.mobileInfo.device === 'iPhone' &&
			player.mobileInfo.userOsMajor !== false &&
			player.mobileInfo.userOsMajor <= 9;
	...
	generateCustomControlTags: function() {
		return (
			...
			'   <div id="' +
			this.videoPlayerId +
			'_fluid_control_playpause" class="fluid_button fluid_button_play"></div>' +
			'</div>' +
			'<div id="' +
			this.videoPlayerId +
			'_fluid_controls_progress_container" class="fluid_controls_progress_container fluid_slider">' +
			'   <div class="fluid_controls_progress">' +
			'      <div id="' +
			this.videoPlayerId +
			'_vast_control_currentprogress" class="fluid_controls_currentprogress">' +
			'          <div id="' +
			this.videoPlayerId +
			'_vast_control_currentpos" class="fluid_controls_currentpos"></div>' +
			'      </div>' +
			'   </div>' +
			'   <div id="' +
			this.videoPlayerId +
			'_buffered_amount" class="fluid_controls_buffered"></div>' +
			'   <div id="' +
			this.videoPlayerId +
			'_ad_markers_holder" class="fluid_controls_ad_markers_holder"></div>' +
			'</div>' +
			'<div class="fluid_controls_right">' +
			'   <div id="' +
			this.videoPlayerId +
			'_fluid_control_fullscreen" class="fluid_button fluid_button_fullscreen"></div>' +
			'   <div id="' +
			this.videoPlayerId +
			'_fluid_control_theatre" class="fluid_button fluid_button_theatre"></div>' +
			'   <div id="' +
			this.videoPlayerId +
			'_fluid_control_video_source" class="fluid_button fluid_button_video_source"></div>' +
			'   <div id="' +
			this.videoPlayerId +
			'_fluid_control_playback_rate" class="fluid_button fluid_button_playback_rate"></div>' +
			'   <div id="' +
			this.videoPlayerId +
			'_fluid_control_download" class="fluid_button fluid_button_download"></div>' +
			'   <div id="' +
			this.videoPlayerId +
			'_fluid_control_volume_container" class="fluid_control_volume_container fluid_slider">' +
			'       <div id="' +
			this.videoPlayerId +
			'_fluid_control_volume" class="fluid_control_volume">' +
			'           <div id="' +
			this.videoPlayerId +
			'_fluid_control_currentvolume" class="fluid_control_currentvolume">' +
			'               <div id="' +
			this.videoPlayerId +
			'_fluid_control_volume_currentpos" class="fluid_control_volume_currentpos"></div>' +
			'           </div>' +
			'       </div>' +
			'   </div>' +
			'   <div id="' +
			this.videoPlayerId +
			'_fluid_control_mute" class="fluid_button fluid_button_volume"></div>' +
			'   <div id="' +
			this.videoPlayerId +
			'_fluid_control_duration" class="fluid_fluid_control_duration">00:00 / 00:00</div>' +
			'</div>'
		);
	},
	...
	playPauseAnimationToggle: function(play) {
		...
		if (play) {
			document
				.getElementById(videoPlayerId + '_fluid_state_button')
	...
	contolDurationUpdate: function(videoPlayerId) {
		...
		var videoPlayerTag = document.getElementById(videoPlayerId);
		var durationText =
			player.pad(parseInt(videoPlayerTag.currentTime / 60)) +
			':' +
			player.pad(parseInt(videoPlayerTag.currentTime % 60)) +
			' / ' +
			player.pad(parseInt(player.currentVideoDuration / 60)) +
			':' +
			player.pad(parseInt(player.currentVideoDuration % 60));
	...
	fullscreenOff: function(fullscreenButton, menuOptionFullscreen) {
		fullscreenButton.className = fullscreenButton.className.replace(
	...
	getEventOffsetY: function(evt, el) {
		...
		if (videoWrapper) {
			...
			var requestFullscreenFunctionNames = fluidPlayerClass.checkFullscreenSupport(
				'fluid_video_wrapper_' + videoPlayerId
			);
	...
	onProgressbarMouseDown: function(videoPlayerId, event) {
		...
		var initialPosition = fluidPlayerClass.getEventOffsetX(
			event,
			document.getElementById(videoPlayerId + '_fluid_controls_progress_container')
		);
		...
		function onProgressbarMouseMove(event) {
			var currentX = fluidPlayerClass.getEventOffsetX(
				event,
				document.getElementById(videoPlayerId + '_fluid_controls_progress_container')
			);
		...
		function onProgressbarMouseUp(event) {
			...
			var clickedX = fluidPlayerClass.getEventOffsetX(
				event,
				document.getElementById(videoPlayerId + '_fluid_controls_progress_container')
			);
	...
	onVolumebarMouseDown: function(videoPlayerId) {
		...
		function onVolumebarMouseMove(event) {
			var currentX = fluidPlayerClass.getEventOffsetX(
				event,
				document.getElementById(videoPlayerId + '_fluid_control_volume_container')
			);
	...
	setVastList: function() {
		...
		var validateRequiredParams = function(item) {
			...
			if (item.size && player.supportedNonLinearAd.indexOf(item.size) === -1) {
				player.announceLocalError(
					102,
					'Only ' + player.supportedNonLinearAd.join(',') + ' size are supported.'
				);
				hasError = true;
			}
	...
	currentTimeChange: function(videoPlayerId, keyCode) {
		...
		videoPlayerTag.currentTime = videoInstanceId.getNewCurrentTimeValueByKeyCode(
			keyCode,
			videoPlayerTag.currentTime,
			videoPlayerTag.duration
		);
	},
	...
	handleMouseleave: function(event) {
		...
		if (
			...
			document
				.getElementById('fluid_video_wrapper_' + videoInstanceId)
	...
	keyboardControl: function() {
		...
		window.addEventListener('click', function(e) {
			...
			if (
				document.getElementById('fluid_video_wrapper_' + videoInstanceId).contains(e.target) ||
	...
	playPauseToggle: function(videoPlayerTag) {
		...
		if (!isFirstStart || preRolls.length == 0) {
			...
			if (player.displayOptions.layoutControls.layout !== 'browser') {
				//The original player play/pause toggling is managed by the browser
				if (videoPlayerTag.paused) {
			...
			if (
				...
				(player.mobileInfo.userOs !== false &&
					player.mobileInfo.userOs == 'Android' &&
	...
	setCustomControls: function() {
		...
		document.getElementById(this.videoPlayerId + '_fluid_control_playpause').addEventListener(
			'click',
			function() {
				if (!player.firstPlayLaunched) {
					videoPlayerTag.removeEventListener('play', player.initialPlay);
		...
		document.getElementById(player.videoPlayerId).addEventListener(
			'play',
			function() {
		...
		document.getElementById(player.videoPlayerId + '_fluid_controls_progress_container').addEventListener(
			eventOn,
			function(event) {
		...
		document.getElementById(player.videoPlayerId + '_fluid_control_volume_container').addEventListener(
			eventOn,
			function(event) {
		...
		document
			.getElementById(player.videoPlayerId + '_fluid_control_fullscreen')
		...
		if (player.displayOptions.layoutControls.allowTheatre && !player.isInIframe) {
			document
				.getElementById(player.videoPlayerId + '_fluid_control_theatre')
				.addEventListener('click', function() {
	...
	setCustomContextMenu: function() {
		...
		divContextMenu.innerHTML =
			...
			'    <li id="' +
			player.videoPlayerId +
			'context_option_play">Play</li>' +
			'    <li id="' +
			player.videoPlayerId +
			'context_option_mute">Mute</li>' +
			'    <li id="' +
			player.videoPlayerId +
			'context_option_fullscreen">Fullscreen</li>' +
			'    <li id="' +
			player.videoPlayerId +
			'context_option_homepage">Fluid Player ' +
			player.version +
			'</li>' +
			'</ul>';
		...
		menuOptionPlay.addEventListener(
			'click',
			function() {
				player.playPauseToggle(videoPlayerTag);
			},
		...
		menuOptionMute.addEventListener(
			'click',
			function() {
				player.muteToggle(player.videoPlayerId);
			},
		...
		menuOptionFullscreen.addEventListener(
			'click',
			function() {
				player.fullscreenToggle();
			},
		...
		menuOptionHomepage.addEventListener(
			'click',
			function() {
			...
			function() {
				var convertVttRawData = function(vttRawData) {
					...
					for (var i = 0; i < vttRawData.cues.length; i++) {
						...
						if (
							player.displayOptions.layoutControls.timelinePreview.spriteRelativePath &&
							player.displayOptions.layoutControls.timelinePreview.file.indexOf('/') !== -1 &&
							(typeof player.displayOptions.layoutControls.timelinePreview.sprite === 'undefined' ||
								player.displayOptions.layoutControls.timelinePreview.sprite == '')
						) {
							imageUrl = player.displayOptions.layoutControls.timelinePreview.file.substring(
								0,
								player.displayOptions.layoutControls.timelinePreview.file.lastIndexOf('/')
							);
							imageUrl += '/' + tempThumbnailData[0];
						} else {
							imageUrl = player.displayOptions.layoutControls.timelinePreview.sprite
								? player.displayOptions.layoutControls.timelinePreview.sprite
								: tempThumbnailData[0];
						}
	...
	getThumbnailCoordinates: function(second) {
		...
		if (player.timelinePreviewData.length) {
			for (var i = 0; i < player.timelinePreviewData.length; i++) {
				if (
					second >= player.timelinePreviewData[i].startTime &&
					second <= player.timelinePreviewData[i].endTime
				) {
	...
	drawTimelinePreview: function(event) {
		...
		var timelinePreviewShadow = document.getElementById(
			player.videoPlayerId + '_fluid_timeline_preview_container_shadow'
		);
		var progressContainer = document.getElementById(player.videoPlayerId + '_fluid_controls_progress_container');
		var totalWidth = progressContainer.clientWidth;
		...
		if (
			...
			switch (player.displayOptions.layoutControls.timelinePreview.type) {
				case 'VTT':
					...
					document
						.getElementById(player.videoPlayerId + '_fluid_controls_progress_container')
					...
					document
						...
						.addEventListener(
							...
							function(event) {
								...
								if (
									typeof event.clientX !== 'undefined' &&
									progress.contains(document.elementFromPoint(event.clientX, event.clientY))
								) {
									//False positive (Chrome bug when fast click causes leave event)
									return;
								}
								document.getElementById(
									player.videoPlayerId + '_fluid_timeline_preview_container'
								).style.display =
									'none';
								document.getElementById(
									player.videoPlayerId + '_fluid_timeline_preview_container_shadow'
	...
	setupPlayerWrapper: function() {
		...
		divVideoWrapper.className = fluidPlayerClass.isTouchDevice()
	...
	userActivityChecker: function() {
		...
		var listenTo = fluidPlayerClass.isTouchDevice()
	...
	hideControlBar: function() {
		...
		if (
			videoPlayerInstance.isCurrentlyPlayingAd &&
			!videoPlayerTag.paused &&
			videoPlayerInstance.displayOptions.layoutControls.layout !== 'browser'
		) {
			videoPlayerInstance.toggleAdCountdown(true);
		}

		if (videoPlayerInstance.hasControlBar()) {
			var divVastControls = document.getElementById(
				videoPlayerInstance.videoPlayerId + '_fluid_controls_container'
			);
				...
				for (var i = 0; i < videoPlayer.buffered.length; i++) {
					if (videoPlayer.buffered.start(videoPlayer.buffered.length - 1 - i) < videoPlayer.currentTime) {
						bufferBar.style.width =
							videoPlayer.buffered.end(videoPlayer.buffered.length - 1 - i) / duration * 100 + '%';
	...
	theatreToggle: function() {
		...
		if (this.displayOptions.layoutControls.theatreAdvanced) {
			var elementForTheatre = document.getElementById(
				this.displayOptions.layoutControls.theatreAdvanced.theatreElement
			);
				...
				case 'center':
					...
					if (typeof workingWidth == 'string' && workingWidth.substr(workingWidth.length - 1) == '%') {
						defaultHorizontalMargin =
							(100 - parseInt(workingWidth.substring(0, workingWidth.length - 1))) / 2 + '%'; // A margin of half the remaining space
					} else if (
						typeof workingWidth == 'string' &&
						workingWidth.substr(workingWidth.length - 2) == 'px'
					) {
						defaultHorizontalMargin =
	...
	posterImage: function() {
		if (this.displayOptions.layoutControls.posterImage) {
			...
			containerDiv.style.background =
				"url('" + this.displayOptions.layoutControls.posterImage + "') center center / contain no-repeat black";
		...
		switch (this.displayOptions.layoutControls.mediaType) {
			case 'application/dash+xml': // MPEG-DASH
				if (!this.dashScriptLoaded) {
					// First time trying adding in DASH streamer, get the script
	...
	setPersistentSettings: function() {
		if (typeof Storage !== 'undefined' && typeof localStorage !== 'undefined') {
			...
			if (
				typeof this.fluidStorage.fluidVolume !== 'undefined' &&
			...
			if (
				typeof this.fluidStorage.fluidQuality !== 'undefined' &&
				this.displayOptions.layoutControls.persistentSettings.quality
			) {
				var sourceOption = document.getElementById(
					'source_' + this.videoPlayerId + '_' + this.fluidStorage.fluidQuality
				);
			...
			if (
				typeof this.fluidStorage.fluidSpeed !== 'undefined' &&
			...
			if (
				typeof this.fluidStorage.fluidTheatre !== 'undefined' &&
				this.fluidStorage.fluidTheatre == 'true' &&
	...
	toggleLogo: function(logo) {
		...
		this.displayOptions.layoutControls.logo.mouseOverImageUrl = logo.mouseOverImageUrl
			? logo.mouseOverImageUrl
			: null;
		this.displayOptions.layoutControls.logo.imageMargin = logo.imageMargin ? logo.imageMargin : '2px';
		this.displayOptions.layoutControls.logo.hideWithControls = logo.hideWithControls
